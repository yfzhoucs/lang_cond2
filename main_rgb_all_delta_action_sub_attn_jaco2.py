...
def train(writer, name, epoch_idx, data_loader, model, 
    ...
    for idx, (img, target, joint_angles, ee_pos, ee_traj, ee_xy, length, target_pos, phis, mask, target_xy, sentence, joint_angles_traj, displacement) in enumerate(data_loader):
        ...
        print(attn_index_tar)
        print(attn_map2.shape)
...
def main(writer, name, batch_size=256):
    ...
    # load data
    data_dirs = [
        os.path.join(data_root_path, 'dataset/mujoco_dataset_pick_push_RGBD_different_angles_fast_gripper_224_jaco2/'),
    ]
    ...
    dataset_test = DMPDatasetEERandTarXYLang([os.path.join(data_root_path, 'dataset/mujoco_dataset_pick_push_RGBD_different_angles_fast_gripper_224_jaco2_test/')], random=True, length_total=120, normalize='jaco2')
    ...
    data_loader_train_dmp = torch.utils.data.DataLoader(dataset_train_dmp, batch_size=batch_size,
                                          shuffle=True, num_workers=8,
                                          collate_fn=pad_collate_xy_lang)
    dataset_test_dmp = DMPDatasetEERandTarXYLang([os.path.join(data_root_path, 'dataset/mujoco_dataset_pick_push_RGBD_different_angles_fast_gripper_224_jaco2_test/')], random=False, length_total=120, normalize='jaco2')
...
if __name__ == '__main__':
    name = 'train-12-rgb-sub-attn-fast-gripper-jaco2'
