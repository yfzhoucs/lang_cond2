...
def main(writer, name, batch_size=256):
    ...
    pretrained_dict = torch.load(os.path.join(ckpt_path, 'train-12-rgbd-mse-displacement-lr-1e-4-aligned-train-test-centered/405000.pth'))['model']
    ...
    # for param in model.needs_gradient_paramlist:
    #     param.requires_grad = True
    # for param in model.needs_gradient_modulelist.parameters():
    #     param.requires_grad = True
    ...
    # load data
    data_dirs = [
        os.path.join(data_root_path, 'dataset/mujoco_dataset_pick_push_RGBD_different_angles_224_panda_aligned_light/'),
    ]
    ...
    dataset_test = DMPDatasetEERandTarXYLang([os.path.join(data_root_path, 'dataset/mujoco_dataset_pick_push_RGBD_different_angles_224_panda_aligned_light_test/')], random=True, length_total=120, normalize='panda')
    data_loader_test = torch.utils.data.DataLoader(dataset_test, batch_size=batch_size,
                                          shuffle=True, num_workers=8,
                                          collate_fn=pad_collate_xy_lang)
    dataset_train_split = DMPDatasetEERandTarXYLang([os.path.join(data_root_path, 'dataset/mujoco_dataset_pick_push_RGBD_different_angles_224_panda_aligned_light_train_split/')], random=True, length_total=120, normalize='panda')
    ...
    data_loader_train_dmp = torch.utils.data.DataLoader(dataset_train_dmp, batch_size=batch_size,
                                          shuffle=True, num_workers=8,
                                          collate_fn=pad_collate_xy_lang)
    dataset_test_dmp = DMPDatasetEERandTarXYLang([os.path.join(data_root_path, 'dataset/mujoco_dataset_pick_push_RGBD_different_angles_224_panda_aligned_light_test/')], random=False, length_total=120, normalize='panda')
    data_loader_test_dmp = torch.utils.data.DataLoader(dataset_test_dmp, batch_size=batch_size,
                                          shuffle=True, num_workers=8,
                                          collate_fn=pad_collate_xy_lang)
    dataset_train_split_dmp = DMPDatasetEERandTarXYLang([os.path.join(data_root_path, 'dataset/mujoco_dataset_pick_push_RGBD_different_angles_224_panda_aligned_light_train_split/')], random=False, length_total=120, normalize='panda')
...
if __name__ == '__main__':
    name = 'train-12-rgbd-mse-displacement-lr-1e-4-aligned-train-test-panda-transfer-from-ur5-aligned-depth-light-not-loading-query-2-amsgrad'
