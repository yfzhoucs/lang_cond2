...
from utils.load_data_rgb_all_delta_action_real import DMPDatasetEERandTarXYLang, pad_collate_xy_lang
...
def train(writer, name, epoch_idx, data_loader, model, 
    ...
    for idx, (img, target, joint_angles, ee_pos, ee_traj, ee_xy, length, target_pos, phis, mask, target_xy, sentence, joint_angles_traj, displacement) in enumerate(data_loader):
        ...
        if stage >= 1:
            ...
            supervision_layer3 = [[1, [0, 2, 3]], [2, [2, 3]], [4, [4]]]
...
def main(writer, name, batch_size=256):
    ...
    ckpt = os.path.join(ckpt_path, 'train-12-rgb-all-delta-action/445000.pth')
    ...
    # load data
    data_dirs = [
        os.path.join(data_root_path, 'dataset/mujoco_dataset_pick_push_RGBD_different_angles_224_real/'),
    ]
    ...
    dataset_test = DMPDatasetEERandTarXYLang([os.path.join(data_root_path, 'dataset/mujoco_dataset_pick_push_RGBD_different_angles_224_real_test/')], random=True, length_total=120)
    data_loader_test = torch.utils.data.DataLoader(dataset_test, batch_size=batch_size,
                                          shuffle=True, num_workers=8,
                                          collate_fn=pad_collate_xy_lang)
    # dataset_train_split = DMPDatasetEERandTarXYLang([os.path.join(data_root_path, 'dataset/mujoco_dataset_pick_push_RGBD_different_angles_224_real_test/')], random=True, length_total=120)
    ...
    data_loader_train_dmp = torch.utils.data.DataLoader(dataset_train_dmp, batch_size=batch_size,
                                          shuffle=True, num_workers=8,
                                          collate_fn=pad_collate_xy_lang)
    dataset_test_dmp = DMPDatasetEERandTarXYLang([os.path.join(data_root_path, 'dataset/mujoco_dataset_pick_push_RGBD_different_angles_224_real_test/')], random=False, length_total=120)
    data_loader_test_dmp = torch.utils.data.DataLoader(dataset_test_dmp, batch_size=batch_size,
                                          shuffle=True, num_workers=8,
                                          collate_fn=pad_collate_xy_lang)
    # dataset_train_split_dmp = DMPDatasetEERandTarXYLang([os.path.join(data_root_path, 'dataset/mujoco_dataset_pick_push_RGBD_different_angles_224_real_test/')], random=False, length_total=120)
    # data_loader_train_split_dmp = torch.utils.data.DataLoader(dataset_train_split_dmp, batch_size=batch_size,
    #                                       shuffle=True, num_workers=8,
    #                                       collate_fn=pad_collate_xy_lang)

    optimizer = optim.Adam(model.parameters(), lr=1e-4)
    optimizer.load_state_dict(ckpt_instance['optimizer'])
...
if __name__ == '__main__':
    name = 'train-12-rgb-all-delta-action-sim2real'
