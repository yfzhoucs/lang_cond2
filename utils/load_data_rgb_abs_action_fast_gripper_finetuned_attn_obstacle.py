...
class DMPDatasetEERandTarXYLang(Dataset):
    def __init__(self, data_dirs, random=True, normalize='separate', length_total=91, depth_scale=1000.):
        ...
        for trial in all_dirs:
            ...
            if 'basketball' not in states_dict[0]['objects_to_track']:
                trial_dict['basketball'] = np.asarray([[0, 0, 0] for i in range(trial_dict['len'])])
            else:
                trial_dict['basketball'] = np.asarray([states_dict[i]['objects_to_track']['basketball']['xyz'] for i in range(trial_dict['len'])])
    ...
    def __getitem__(self, index):
        ...
        obstacle_pos = torch.tensor((self.trials[trial_idx]['basketball'][step_idx] - self.mean[:3]) / (self.var[:3] ** (1/2)), dtype=torch.float32)
        obstacle_xy = self.xyz_to_xy(self.trials[trial_idx]['basketball'][step_idx][:3])
        ...
        return img, target, joint_angles, ee_pos, ee_traj, ee_xy, length, target_pos, phis, mask, target_xy, sentence[0], joint_angles_traj, displacement, obstacle_pos, obstacle_xy#, displacement_traj


def pad_collate_xy_lang(batch):
    (img, target, joint_angles, ee_pos, ee_traj, ee_xy, length, target_pos, phis, mask, target_xy, sentence, joint_angles_traj, displacement, obstacle_pos, obstacle_xy) = zip(*batch)
    ...
    obstacle_pos = torch.stack(obstacle_pos)
    obstacle_xy = np.stack(obstacle_xy)

    return  img, target, joint_angles, ee_pos, ee_traj, ee_xy, length, target_pos, phis, mask, target_xy, sentence, joint_angles_traj, displacement, obstacle_pos, obstacle_xy
