...
class DMPDatasetEERandTarXYLang(Dataset):
    def __init__(self, data_dirs, random=True, normalize='separate', length_total=91, depth_scale=1000.):
        ...
        for trial in all_dirs:
            ...
            states_json = os.path.join(trial, 'processed_states.json')
            ...
            trial_dict['stage'] = np.asarray([states_dict[i]['stage'] for i in range(trial_dict['len'])])
            trial_dict['img_paths'] = [os.path.join(trial, 'real_' + str(i) + '_processed.png') for i in range(trial_dict['len'])]
    ...
    def preprocess_tarpos(self, stage, action, target_pos, ee_pos, img_path):
        mask_out = False
        if action == 'push' and stage == 1:
            mask_out = True
        if action == 'put_down' and stage >= 6:
            mask_out = True
        if mask_out:
            tarpos_mask = torch.tensor(0, dtype=torch.int64)
        else:
            tarpos_mask = torch.tensor(1, dtype=torch.int64)

        replace_tarpos_by_eepos = False
        if action == 'pick' and stage == 1 and 'unmatch' in img_path:
            replace_tarpos_by_eepos = True
        if action == 'put_down' and stage in [3, 4, 5] and 'unmatch' in img_path:
            replace_tarpos_by_eepos = True
        if replace_tarpos_by_eepos:
            target_pos = ee_pos

        return target_pos, tarpos_mask
    ...
    def __getitem__(self, index):
        ...
        stage = self.trials[trial_idx]['stage'][step_idx]
        target_pos, mask_tarpos = self.preprocess_tarpos(stage, action, target_pos, ee_pos, self.trials[trial_idx]['img_paths'][step_idx])

        return img, target, joint_angles, ee_pos, ee_traj, ee_xy, length, target_pos, phis, mask, target_xy, sentence[0], joint_angles_traj, displacement, mask_tarpos#, displacement_traj


def pad_collate_xy_lang(batch):
    (img, target, joint_angles, ee_pos, ee_traj, ee_xy, length, target_pos, phis, mask, target_xy, sentence, joint_angles_traj, displacement, mask_tarpos) = zip(*batch)
    ...
    mask_tarpos = torch.stack(mask_tarpos)

    return  img, target, joint_angles, ee_pos, ee_traj, ee_xy, length, target_pos, phis, mask, target_xy, sentence, joint_angles_traj, displacement, mask_tarpos


if __name__ == '__main__':
    data_dirs = [
        '/mnt/disk1/dataset/data_real_only_tarpos_unmatched_q/',
        '/mnt/disk1/dataset/data_real_matched_q_grid_2/',
    ...
    for img, target, joint_angles, ee_pos, ee_traj, ee_xy, length, target_pos, phis, mask, target_xy, sentence, joint_angles_traj, displacement, mask_tarpos in dataloader:
