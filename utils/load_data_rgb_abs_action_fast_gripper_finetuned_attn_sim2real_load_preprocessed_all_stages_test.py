...
class DMPDatasetEERandTarXYLang(Dataset):
    def __init__(self, data_dirs, random=True, normalize='separate', length_total=91, depth_scale=1000.):
        ...
        for trial in all_dirs:
            ...
            if self.random:
                length_of_stage_0 = self.get_stages(states_dict)[0]
                states_dict = states_dict[:length_of_stage_0]
    ...
    def get_stages(self, states_dict):
        stages = [0] * 8
        for i in range(len(states_dict)):
            stage = int(states_dict[i]['stage'])
            stages[stage] += 1
        return stages
    ...
    def __getitem__(self, index):
        ...
        sentence_txt = self.sentence_template(target, action)
        sentence = clip.tokenize([sentence_txt])
        idx_to_name = {
            0: 'target2',
            1: 'coke',
            2: 'pepsi',
        ...
        return img, target, joint_angles, ee_pos, ee_traj, ee_xy, length, target_pos, phis, mask, target_xy, sentence[0], joint_angles_traj, displacement, sentence_txt, target_txt, action_txt#, displacement_traj


def pad_collate_xy_lang(batch):
    (img, target, joint_angles, ee_pos, ee_traj, ee_xy, length, target_pos, phis, mask, target_xy, sentence, joint_angles_traj, displacement, sentence_txt, target_txt, action_txt) = zip(*batch)
    ...
    return  img, target, joint_angles, ee_pos, ee_traj, ee_xy, length, target_pos, phis, mask, target_xy, sentence, joint_angles_traj, displacement, sentence_txt, target_txt, action_txt
