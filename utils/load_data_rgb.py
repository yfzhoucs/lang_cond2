...
class DMPDatasetEERandTarXYLang(Dataset):
    def __init__(self, data_dirs, random=True, normalize='separate', length_total=91, depth_scale=1000.):
        ...
        # all_joints = np.concatenate([self.trials[idx]['joint_angles'] for idx in range(len(self.trials))])
        # print(all_joints.shape)
        # print(np.mean(all_joints, axis=0))
        # print(np.var(all_joints, axis=0))
        # print('mean, together', np.mean(all_joints))
        # print('var, together', np.var(all_joints))
        # exit()

        # self.mean = np.array([ 2.97563984e-02,  4.47217117e-01,  8.45049397e-02, -1.16634793e-03, -1.49604188e-04, -1.14954971e-03])
        # self.var = np.array([4.52914246e-02, 5.01675921e-03, 4.19371463e-03, 3.69940238e-06, 4.66130096e-07, 1.00764180e-05])
        ...
        #     print(self.trials[i]['EE_xyzrpy'].shape)
        #     plt.plot(self.trials[i]['EE_xyzrpy'][:, 3:])
        #     plt.show()
        # exit()
    ...
    def __getitem__(self, index):
        ...
        return img, target, joint_angles, ee_pos, ee_traj, length, target_pos, phis, mask, target_xy, sentence[0], joint_angles_traj


def pad_collate_xy_lang(batch):
    (img, target, joint_angles, ee_pos, ee_traj, length, target_pos, phis, mask, target_xy, sentence, joint_angles_traj) = zip(*batch)
    ...
    return img, target, joint_angles, ee_pos, ee_traj, length, target_pos, phis, mask, target_xy, sentence, joint_angles_traj


if __name__ == '__main__':
    data_dirs = [
        '/share/yzhou298/dataset/mujoco_dataset_pick_push_RGBD_different_angles_224/'
    ]
