...
class DMPDatasetEERandTarXYLang(Dataset):
    def __init__(self, data_dirs, random=True, normalize='separate', length_total=91, depth_scale=1000.):
        ...
        for trial in all_dirs:
            ...
            trial_dict['img_paths_real'] = [os.path.join(trial, 'real_' + str(i) + '.png') for i in range(trial_dict['len'])]
        ...
        # shift_x = 9
        # shift_y = -15
        # img_real = img_real[-1000:, 350: 1350]
        # img_real = transform.rotate(img_real, 2.3)
        # affine_transform = transform.AffineTransform(translation=(shift_x, shift_y), scale=(1.12, 1.15))
        # img_real = transform.warp(img_real, affine_transform)
        # img_real = transform.resize(img_real, (224, 224))   
        # img_real[:, :24] = 0
    ...
    def plot_7_objects(self, ax, curr_dict, step_idx):
        for obj in ['EE_xyzrpy', 'target2', 'coke', 'pepsi', 'milk', 'bread', 'bottle']:
            print(curr_dict[obj][step_idx][:3])
            xy = self.xyz_to_xy(curr_dict[obj][step_idx][:3])
            xy[1] = 224 - xy[1]
            circle = plt.Circle(xy, 3.5)
            ax.add_patch(circle)

    # # https://stackoverflow.com/questions/47961447/shift-image-in-scikit-image-python        
    # def shift(self, image, translation):
    #     affine_transform = transform.AffineTransform(translation=translation)
    #     # shifted = transform.warp(image, affine_transform, mode='wrap', preserve_range=True)

    #     # shifted = shifted.astype(image.dtype)

    def sim2real_img_transform(self, img_real):
        shift_x = -50
        shift_y = -30
        img_real = TF.rotate(img_real, 2.3)
        img_real = TF.crop(img_real, 70, 355, 893 * 1.28, 870 * 1.28)
        # affine_transform = transform.AffineTransform(translation=(shift_x, shift_y), scale=(1.12, 1.15))
        # img_real = transform.warp(img_real, affine_transform)
        # img_real = TF.resize(img_real, [112, 115])

        # transform_resize = transforms.Resize(size = (893,870))
        # img_real = transform_resize(img_real)
        # img_real = TF.affine(img_real, translate=(shift_x, shift_y), scale=1, angle=0, shear=0)
        img_real = TF.resize(img_real, (224, 224))
        # img_real[:, :24] = 0
        return img_real

    def vis_sim2real_imgs(self, img_sim, img_real, curr_dict, step_idx):
        # shift_x = 9
        # shift_y = 0
        # img_real = img_real[80 + shift_y: 1080 + shift_y, 350 + shift_y: 1350 + shift_y]
        # img_real = self.sim2real_img_transform(img_real)
        fig = plt.figure()
        ax1 = fig.add_subplot(1,2,1)
        self.plot_7_objects(ax1, curr_dict, step_idx)
        ax1.imshow(img_sim)
        ax2 = fig.add_subplot(1,2,2)
        self.plot_7_objects(ax2, curr_dict, step_idx)
        ax2.imshow(img_real)
        plt.show()
        exit()
    ...
    def __getitem__(self, index):
        ...
        # img_real = torch.tensor(io.imread(self.trials[trial_idx]['img_paths_real'][step_idx])[::-1,:,:3] / 255, dtype=torch.float32)
        img = io.imread(self.trials[trial_idx]['img_paths'][step_idx])
        # img_real = io.imread(self.trials[trial_idx]['img_paths_real'][step_idx])

        img_real = io.imread(self.trials[trial_idx]['img_paths_real'][step_idx])
        img_real = self.transform(img_real)
        img_real = self.sim2real_img_transform(img_real)
        print(self.trials[trial_idx]['img_paths_real'][step_idx])
        new_img_path = self.trials[trial_idx]['img_paths_real'][step_idx].split('.')[0] + '_processed.png'
        print(new_img_path)
        img_real.save(new_img_path)


        img_real = TF.vflip(img_real)
        # self.vis_sim2real_imgs(img, img_real, self.trials[trial_idx], step_idx)
        img = TF.to_tensor(img_real)
