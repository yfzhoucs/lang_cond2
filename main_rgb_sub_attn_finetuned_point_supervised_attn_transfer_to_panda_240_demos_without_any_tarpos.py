...
def train(writer, name, epoch_idx, data_loader, model, 
    ...
    for idx, (img, target, joint_angles, ee_pos, ee_traj, ee_xy, length, target_pos, phis, mask, target_xy, sentence, joint_angles_traj, displacement) in enumerate(data_loader):
        ...
        if stage == 0:
            ...
            _, ee_pos_pred, displacement_pred, attn_map, attn_map2, attn_map3 = model(img, joint_angles, sentence, phis, stage)
        else:
            _, ee_pos_pred, displacement_pred, attn_map, attn_map2, attn_map3, attn_map4, trajectory_pred = model(img, joint_angles, sentence, phis, stage)
        ...
        # Attention Loss
        loss_attn = loss_attn_layer1 + loss_attn_layer2 + loss_ee_img_attn 
        loss = 0
        ...
        if stage >= 1:
            ...
            writer.add_scalar('train loss ee pos', loss2.item(), global_step=epoch_idx * len(data_loader) + idx)

            loss = loss2
            loss_attn = loss_attn + loss_attn_layer3

            print(f'{loss_ee_img_attn.item():.2f} {loss_attn_layer1.item():.2f} {loss_attn_layer2.item():.2f} {loss_attn_layer3.item():.2f}')
        ...
        if stage >= 2:
            # Attention Supervision EEF Pos, Command
...
if __name__ == '__main__':
    name = 'train-12-rgb-sub-attn-fast-gripper-abs-action-point-supervised-attn-transfer-to-panda-load-visual-240-demos-without-any-tarpos'
