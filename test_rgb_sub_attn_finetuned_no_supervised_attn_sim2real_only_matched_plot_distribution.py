...
from utils.load_data_rgb_abs_action_fast_gripper_finetuned_attn_sim2real_load_preprocessed_test import DMPDatasetEERandTarXYLang, pad_collate_xy_lang
    ...
    with torch.no_grad():
        ...
        for idx, (img, target, joint_angles, ee_pos, ee_traj, ee_xy, length, target_pos, phis, mask, target_xy, sentence, joint_angles_traj, displacement, sentence_txt, target_txt, action_txt) in enumerate(data_loader):
            ...
            if print_attention_map:
                ...
                plt.plot(idx, (error_trajectory_this_time / num_trajpoints_this_time).item(), action_vis, color=object_vis)

                legend_elements = [
                    # Line2D([0], [0], color='b', lw=4, label='Line'),
                    # Line2D([0], [0], marker='.', color='c', label='pick', markerfacecolor='g', markersize=15),
...
def main(writer, name, batch_size=8):
    ...
    #     # ''
    # ]
    pretrained_dict = torch.load(os.path.join(ckpt_path, 'train-12-rgb-sub-attn-fast-gripper-abs-action-no-supervised-attn-sim2real-only-matched-corrected-json-corrected-matched-folder/20000.pth'))['model']
...
if __name__ == '__main__':
    name = 'test-12-rgb-sub-attn-fast-gripper-abs-action-no-supervised-attn-sim2real-only-matched-corrected-json-corrected-matched-folder-error-distribution'
