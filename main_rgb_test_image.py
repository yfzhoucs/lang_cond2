...
from utils.load_data_rgb import DMPDatasetEERandTarXYLang, pad_collate_xy_lang
    ...
    for idx, (img, target, joint_angles, ee_pos, ee_traj, length, target_pos, phis, mask, target_xy, sentence, joint_angles_traj) in enumerate(data_loader):
        ...
        if save_ckpt:
            ...
            if global_step % 5000 == 0:
                torch.save(model.state_dict(), os.path.join(ckpt_path, name, f'{global_step}.pth'))
...
def main(writer, name, batch_size=96):
    ...
    # load model
    model = Backbone(img_size=224, embedding_size=192, num_traces_in=9, num_traces_out=10, num_weight_points=36, input_nc=3)
    if ckpt is not None:
    ...
    data_dirs = [
        ...
        # os.path.join(data_root_path, 'dataset/mujoco_dataset_pick_push/'),
    ]
    dataset_train = DMPDatasetEERandTarXYLang(data_dirs, random=True, length_total=36)
...
if __name__ == '__main__':
    name = 'train-11-rgb-3-new-dataset'
