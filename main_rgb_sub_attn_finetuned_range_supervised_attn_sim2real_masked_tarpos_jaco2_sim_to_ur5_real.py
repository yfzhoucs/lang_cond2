...
from utils.load_data_rgb_abs_action_fast_gripper_finetuned_attn_sim2real_load_preprocessed_masked_tarpos import DMPDatasetEERandTarXYLang, pad_collate_xy_lang
...
def train(writer, name, epoch_idx, data_loader, model, 
    ...
    for idx, (img, target, joint_angles, ee_pos, ee_traj, ee_xy, length, target_pos, phis, mask, target_xy, sentence, joint_angles_traj, displacement, mask_tarpos) in enumerate(data_loader):
        ...
        mask_tarpos = mask_tarpos.to(device)
        ...
        if stage >= 1:
            mask_tarpos = mask_tarpos.unsqueeze(1).repeat(1, target_position_pred.shape[1])
            target_position_pred = target_position_pred * mask_tarpos
            target_pos = target_pos * mask_tarpos
            displacement_pred = displacement_pred * mask_tarpos
            displacement = displacement * mask_tarpos

            loss0 = criterion(target_position_pred[:, :3], target_pos[:, :3])
            loss1 = criterion(displacement_pred[:, :3], displacement[:, :3])
...
def main(writer, name, batch_size=256):
    ...
    pretrained_dict = torch.load(os.path.join(ckpt_path, 'train-12-rgb-sub-attn-fast-gripper-abs-action-jaco2/470000.pth'))['model']
    ...
    # load data
    data_dirs = [
        '/mnt/disk8/dataset/data_real_matched_q_grid/',
        '/mnt/disk8/dataset/data_real_matched_q/',
        '/mnt/disk8/dataset/data_real_unmatched_q/',
    ]
    data_dirs_val = [
        '/mnt/disk8/dataset/data_real_matched_q_grid_2/',
    ]
    ...
    dataset_val = DMPDatasetEERandTarXYLang(data_dirs_val, random=False, length_total=120, normalize='separate')
...
if __name__ == '__main__':
    name = 'train-12-rgb-sub-attn-fast-gripper-abs-action-range-supervised-attn-sim2real-masked_tarpos-jaco-sim_to_ur5-real'
