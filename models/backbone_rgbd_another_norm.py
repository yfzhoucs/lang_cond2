...
class EEPOSEncoder(nn.Module):
    def __init__(self, num_traces, embedding_size):
        super(EEPOSEncoder, self).__init__()
        self.flatten = nn.Flatten()
        self.layer1 = nn.Linear(num_traces, 256)
        self.layer2 = nn.Linear(256, embedding_size)

    def forward(self, ee_pos):
        x = self.flatten(ee_pos)
        x = F.selu(self.layer1(x))
        x = F.selu(self.layer2(x))
        return x
...
class Backbone(nn.Module):
    def __init__(self, img_size, embedding_size, num_traces_in=3, num_traces_out=4, num_weight_points=91, input_nc=4, device=torch.device('cuda')):
        ...
        # EE POS Pathway
        self.ee_pos_encoder = EEPOSEncoder(num_traces_in, embedding_size)
        self.ee_pos_embed_to_qkv = []
        for i in range(3):
            self.ee_pos_embed_to_qkv.append(nn.Sequential(
                nn.Linear(embedding_size, embedding_size),
                nn.LayerNorm(embedding_size),
                nn.ReLU()
            ))
        self.ee_pos_embed_to_qkv = nn.ModuleList(self.ee_pos_embed_to_qkv)

        # Requests and their mappings to qkv
        self.task_request = nn.Parameter(torch.rand(embedding_size))
        self.ee_request = nn.Parameter(torch.rand(embedding_size))
        self.requests = [self.task_request, self.ee_request]
    ...
    def _ee_pos_pathway_(self, ee_pos):
        ee_pos_embedding = self.ee_pos_encoder(ee_pos).unsqueeze(1)
        ee_pos_embed_query, ee_pos_embed_key, ee_pos_embed_value = self._embed_to_qkv_(ee_pos_embedding, self.task_embed_to_qkv)
        return ee_pos_embed_query, ee_pos_embed_key, ee_pos_embed_value
    ...
    def _update_cortex_status_(self, last_state_embed, perception_query, perception_key, perception_value):
        last_subjective_part = last_state_embed[:, :2, :]
    ...
    def _get_segment_embed_(self, batch_size, embed_shape, layer):
        segment_ids = [0, 1] + embed_shape * [2] + [3, 4]
    ...
    def forward(self, img, ee_pos, sentence, phis, stage=0):
        ...
        # EE POS Pathway
        ee_pos_embed_query, ee_pos_embed_key, ee_pos_embed_value = self._ee_pos_pathway_(ee_pos)

        # Concatenate All Perception Pathways
        perception_query = torch.cat((img_embed_query, ee_pos_embed_query, task_embed_query), dim=1)
        perception_key = torch.cat((img_embed_key, ee_pos_embed_key, task_embed_key), dim=1)
        perception_value = torch.cat((img_embed_value, ee_pos_embed_value, task_embed_value), dim=1)
        ...
        # Attention itself
        segment_embed = self._get_segment_embed_(batch_size=img.shape[0], embed_shape=img_embed_query.shape[1], layer=1)
        cortex_query = cortex_query + segment_embed
        cortex_key = cortex_key + segment_embed
        cortex_value = cortex_value + segment_embed
        state_embedding, attn_map = self.attn(cortex_query[:, :2, :], cortex_key, cortex_value, need_weights=True, attn_mask=None)
        ...
        state_embedding2, attn_map2 = self.attn2(cortex_query2[:, :2, :], cortex_key2, cortex_value2, need_weights=True, attn_mask=None)
        ...
        if stage == 0:
            return target_position_pred, attn_map, attn_map2
        ...
        state_embedding3, attn_map3 = self.attn3(cortex_query3[:, :2, :], cortex_key3, cortex_value3, need_weights=True, attn_mask=None)

        dmp_weights = self.controller(state_embedding3[:, 1, :])
        ...
        return target_position_pred, attn_map, attn_map2, attn_map3, trajectory
