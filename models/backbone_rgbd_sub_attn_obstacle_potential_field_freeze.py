...
class Backbone(nn.Module):
    def __init__(self, img_size, embedding_size, num_traces_in=7, num_traces_out=10, num_weight_points=91, input_nc=4, device=torch.device('cuda')):
        ...
        self.obstacle_slot = nn.Parameter(torch.rand(embedding_size))
        self.requests = [self.tar_pos_slot, self.displace_slot, self.ee_pos_slot, self.ee_pos2_slot, self.action_slot, self.obstacle_slot]
        ...
        # Cortex Module
        self.seg_embed_1 = nn.Embedding(9, embedding_size)
        self.seg_embed_obs = nn.Embedding(9, embedding_size)
        ...
        self.embed_to_obstacle_pos = nn.Sequential(
            nn.Linear(embedding_size, 128), 
            nn.SELU(), 
            nn.Linear(128, 3))

        self.embed_to_potential_field = nn.Sequential(
            nn.Linear(embedding_size, 128), 
            nn.SELU(), 
            nn.Linear(128, 3))
    ...
    def _get_segment_embed_(self, batch_size, embed_shape, layer):
        segment_ids = [0, 1, 2, 3, 4, 8] + embed_shape * [5] + [6, 7]
    ...
    def forward(self, img, joints, sentence, phis, stage=0):
        ...
        obstacle_pos_pred = self.embed_to_obstacle_pos(state_embedding2[:, 5, :])
        if stage == 0:
            return attn_map, attn_map2, obstacle_pos_pred
        ...
        potential_vector_pred = self.embed_to_potential_field(state_embedding3[:, 5, :])
        if stage == 1:
            return target_position_pred, ee_pos_pred, displacement_pred, obstacle_pos_pred, potential_vector_pred, attn_map, attn_map2, attn_map3
        ...
        return target_position_pred, ee_pos_pred, displacement_pred, obstacle_pos_pred, potential_vector_pred, attn_map, attn_map2, attn_map3, attn_map4, trajectory
