...
class TransformerOutput(nn.Module):
    def __init__(self, embedding_size, layer_norm_eps=1e-12, hidden_dropout_prob=0.1):
        super().__init__()
...
class ResidualAttention(nn.Module):
    def __init__(self, device, embedding_size, layer_norm_eps=1e-12):
        super().__init__()
        self.layernorm = nn.LayerNorm(embedding_size, eps=layer_norm_eps)
    ...
    def forward(self, q, k, v):
        q2 = self.layernorm(q)
        q3, attn_map = self.attn(q2, k, v, need_weights=True, attn_mask=None)
        return q + q3, attn_map


class ResidualFFN(nn.Module):
    def __init__(self, embedding_size, layer_norm_eps=1e-12):
        super().__init__()
        self.layernorm = nn.LayerNorm(embedding_size, eps=layer_norm_eps)
    ...
    def forward(self, hidden_states):
        hidden_states2 = self.layernorm(hidden_states)
        hidden_states2 = self.w1(hidden_states2)
        hidden_states2 = self.act(hidden_states2)
        hidden_states2 = self.w2(hidden_states2)
...
class AttentionLayer(nn.Module):
    def __init__(self, device, embedding_size, layer_norm_eps=1e-12):
        super().__init__()
        self.residual_attention = ResidualAttention(device, embedding_size, layer_norm_eps)
        self.residual_ffn = ResidualFFN(embedding_size, layer_norm_eps)

    def forward(self, q, k, v):
        hidden_states, attn_map = self.residual_attention(q, k, v)
        hidden_states = self.residual_ffn(hidden_states)
...
class Backbone(nn.Module):
    def __init__(self, img_size, embedding_size, num_traces_in=7, num_traces_out=10, num_weight_points=91, input_nc=4, device=torch.device('cuda')):
        ...
        # self.attn1 = nn.MultiheadAttention(embed_dim=embedding_size, num_heads=8, device=device, batch_first=True)
        ...
        # self.output11 = TransformerOutput(embedding_size)
        # self.output21 = TransformerOutput(embedding_size)
        # self.output31 = TransformerOutput(embedding_size)
        # self.output41 = TransformerOutput(embedding_size)
        # self.output12 = TransformerOutput(embedding_size)
        # self.output22 = TransformerOutput(embedding_size)
        # self.output32 = TransformerOutput(embedding_size)
        # self.output42 = TransformerOutput(embedding_size)
        # self.intermediate1 = TransformerIntermediate(embedding_size)
        # self.intermediate2 = TransformerIntermediate(embedding_size)
        # self.intermediate3 = TransformerIntermediate(embedding_size)
        # self.intermediate4 = TransformerIntermediate(embedding_size)
        self.attn1 = AttentionLayer(device, embedding_size)
        self.attn2 = AttentionLayer(device, embedding_size)
        self.attn3 = AttentionLayer(device, embedding_size)
        self.attn4 = AttentionLayer(device, embedding_size)
    ...
    def _requests_pathway_(self, batch_size):
        ...
        # requests_vs = []
        for i in range(len(self.requests)):
            requests_qs.append(self.requests[i].unsqueeze(0).unsqueeze(1).repeat(batch_size, 1, 1))
            # requests_ks.append(self.requests[i].unsqueeze(0).unsqueeze(1).repeat(batch_size, 1, 1))
            # requests_vs.append(self.requests[i].unsqueeze(0).unsqueeze(1).repeat(batch_size, 1, 1))
    ...
    def _joints_pathway_(self, joints):
        ...
        return joints_embedding#, joints_embedding, joints_embedding
    ...
    def _status_embed_to_qkv_(self, status_embed):
        ...
        return status_embed, status_embed, status_embed
    ...
    # def attn_forward(self, batch_size, embed_shape, requests, perception, attn_layer, out1, intermediate, out2):
    #     kv = torch.cat((requests, perception), dim=1)
    #     state_embedding, attn_map = attn_layer(requests, kv, kv, need_weights=True, attn_mask=None)
    #     state_embedding2 = out1(state_embedding, requests)
    #     state_embedding3 = intermediate(state_embedding2)
    #     state_embedding4 = out2(state_embedding3, state_embedding2)
    #     return state_embedding4, attn_map

    def attn_forward(self, requests, perception, attn_layer):
        kv = torch.cat((requests, perception), dim=1)
        state_embedding, attn_map = attn_layer(requests, kv, kv)
        return state_embedding, attn_map

    def forward(self, img, joints, sentence, phis, stage=0):

        # Image Pathway
        img_embed = self._img_pathway_(img)

        # Task ID Pathway
        task_embed = self._task_id_pathway_(sentence)

        # EE POS Pathway
        joints_embed = self._joints_pathway_(joints)

        # Concatenate All Perception Pathways
        perception = torch.cat((img_embed, joints_embed, task_embed), dim=1)

        # Requests
        requests = self._requests_pathway_(batch_size=img.shape[0])
        ...
        segment_embed = self._get_segment_embed_(batch_size=img.shape[0], embed_shape=img_embed.shape[1], layer=1)
        perception = perception + segment_embed

        # Attention Layer1
        # state_embedding, attn_map = self.attn_forward(
        #     batch_size=img.shape[0], 
        #     embed_shape=img_embed.shape[1], 
        #     requests=requests, 
        #     perception=perception,
        #     out1=self.output11,
        #     intermediate=self.intermediate1,
        #     out2=self.output12,
        #     attn_layer=self.attn1)
        state_embedding, attn_map = self.attn_forward(requests, perception, self.attn1)

        # Attention Layer2
        # state_embedding2, attn_map2 = self.attn_forward(
        #     batch_size=img.shape[0], 
        #     embed_shape=img_embed.shape[1], 
        #     requests=state_embedding, 
        #     perception=perception,
        #     out1=self.output21,
        #     intermediate=self.intermediate2,
        #     out2=self.output22,
        #     attn_layer=self.attn2)
        state_embedding2, attn_map2 = self.attn_forward(state_embedding, perception, self.attn2)
        ...
        ee_pos_from_joints_pred = self.embed_to_ee_pos(state_embedding2[:, 2, :])
        ee_pos_from_img_pred = self.embed_to_ee_pos(state_embedding2[:, 3, :])
        if stage == 0:
            return target_position_pred, ee_pos_from_joints_pred, ee_pos_from_img_pred, attn_map, attn_map2

        # Attention Layer3
        # state_embedding3, attn_map3 = self.attn_forward(
        #     batch_size=img.shape[0], 
        #     embed_shape=img_embed.shape[1], 
        #     requests=state_embedding2, 
        #     perception=perception,
        #     out1=self.output31,
        #     intermediate=self.intermediate3,
        #     out2=self.output32,
        #     attn_layer=self.attn3)
        state_embedding3, attn_map3 = self.attn_forward(state_embedding2, perception, self.attn3)
        # Post-attn operations. Predict the results from the state embedding
        displacement_pred = self.embed_to_displacement(state_embedding3[:, 1, :])
        if stage == 1:
            return target_position_pred, ee_pos_from_joints_pred, ee_pos_from_img_pred, displacement_pred, attn_map, attn_map2, attn_map3

        # Attention Layer4
        # state_embedding4, attn_map4 = self.attn_forward(
        #     batch_size=img.shape[0], 
        #     embed_shape=img_embed.shape[1], 
        #     requests=state_embedding3, 
        #     perception=perception,
        #     out1=self.output41,
        #     intermediate=self.intermediate4,
        #     out2=self.output42,
        #     attn_layer=self.attn4)
        state_embedding4, attn_map4 = self.attn_forward(state_embedding3, perception, self.attn4)
        ...
        return target_position_pred, ee_pos_from_joints_pred, ee_pos_from_img_pred, displacement_pred, attn_map, attn_map2, attn_map3, attn_map4, trajectory
