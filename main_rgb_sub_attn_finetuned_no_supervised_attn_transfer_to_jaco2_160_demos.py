...
def train(writer, name, epoch_idx, data_loader, model, 
    ...
    for idx, (img, target, joint_angles, ee_pos, ee_traj, ee_xy, length, target_pos, phis, mask, target_xy, sentence, joint_angles_traj, displacement) in enumerate(data_loader):
        ...
        if attn_index_tar.min() < 5 or attn_index_tar.max() >= 28 * 28 + 5:
            continue
        # print(img.shape)
...
def main(writer, name, batch_size=256):
    ...
    pretrained_dict = torch.load(os.path.join(ckpt_path, 'train-12-rgb-sub-attn-fast-gripper-abs-action-no-supervised-attn-take2/110000.pth'))['model']
    ...
    # load data
    data_dirs = [
        os.path.join(data_root_path, 'dataset/mujoco_dataset_pick_push_RGBD_different_angles_fast_gripper_224_jaco2_test_0/'),
        os.path.join(data_root_path, 'dataset/mujoco_dataset_pick_push_RGBD_different_angles_fast_gripper_224_jaco2_test_1/'),
        # os.path.join(data_root_path, 'dataset/mujoco_dataset_pick_push_RGBD_different_angles_fast_gripper_224_jaco2_test_2/'),
        # os.path.join(data_root_path, 'dataset/mujoco_dataset_pick_push_RGBD_different_angles_fast_gripper_224_jaco2_test_3/'),
    ]
    dataset_train = DMPDatasetEERandTarXYLang(data_dirs, random=True, length_total=120, normalize='jaco2')
    data_loader_train = torch.utils.data.DataLoader(dataset_train, batch_size=batch_size,
                                          shuffle=True, num_workers=8,
                                          collate_fn=pad_collate_xy_lang)
    dataset_test = DMPDatasetEERandTarXYLang([os.path.join(data_root_path, 'dataset/mujoco_dataset_pick_push_RGBD_different_angles_fast_gripper_224_jaco2_test_4/')], random=True, length_total=120, normalize='jaco2')
    ...
    dataset_train_dmp = DMPDatasetEERandTarXYLang(data_dirs, random=False, length_total=120, normalize='jaco2')
    data_loader_train_dmp = torch.utils.data.DataLoader(dataset_train_dmp, batch_size=batch_size,
                                          shuffle=True, num_workers=8,
                                          collate_fn=pad_collate_xy_lang)
    dataset_test_dmp = DMPDatasetEERandTarXYLang([os.path.join(data_root_path, 'dataset/mujoco_dataset_pick_push_RGBD_different_angles_fast_gripper_224_jaco2_test_4/')], random=False, length_total=120, normalize='jaco2')
...
if __name__ == '__main__':
    name = 'train-12-rgb-sub-attn-fast-gripper-abs-action-no-supervised-attn-transfer-to-jaco2-load-visual-160-demos'
