...
def main(writer, name, batch_size=96):
    ...
    if ckpt is not None:
        model.load_state_dict(torch.load(ckpt)['model'], strict=False)

    model = model.to(device)

    # load data
    data_dirs = [
        os.path.join(data_root_path, 'mujoco_dataset_pick_push_RGBD_different_angles_fast_gripper_224_obstacle/'),
    ]
    data_dirs_test = [
...
if __name__ == '__main__':
    name = 'train-12-rgb-sub-attn-fast-gripper-abs-action-obstacle'
