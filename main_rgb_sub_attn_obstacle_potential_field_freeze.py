...
def main(writer, name, batch_size=96):
    ...
    if ckpt is not None:
        ...
        add_on_embed = torch.zeros(1, seg_embed.shape[1], dtype=torch.float32)
        ...
        if 'seg_embed_obs.weight' not in state_dict:
            ...
            seg_embed_obs = torch.cat((torch.zeros(seg_embed.shape[0] - 1, seg_embed.shape[1]), seg_embed_obs), axis=0)
