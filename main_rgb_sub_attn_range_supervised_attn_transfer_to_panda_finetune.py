...
def main(writer, name, batch_size=256):
    ...
    ckpt = 'train-12-rgb-sub-attn-fast-gripper-abs-action-range-supervised-attn-transfer-to-panda-more-warmup/110000.pth'

    # load model
    model = Backbone(img_size=224, embedding_size=192, num_traces_in=8, num_traces_out=10, num_weight_points=12, input_nc=3)
    if ckpt is not None:
        ckpt = os.path.join(ckpt_path, ckpt)
...
if __name__ == '__main__':
    name = 'train-12-rgb-sub-attn-fast-gripper-abs-action-range-supervised-attn-transfer-to-panda-more-warmup-finetune-1e-5'
